...
angular.module('pruebaApp')
  ...
  .controller('RenderCtrl', ['$scope',function($scope) {
    ...
    VIZ.count = data.length;
      ...
      var margin = {top: 40, right: 0, bottom: 16, left: 20},
    ...
    function setData(d, i) {
      ...
      sphere.position.x = 3100 * Math.cos(theta) * Math.sin(phi);
      sphere.position.y = 3100 * Math.sin(theta) * Math.sin(phi);
      sphere.position.z = 3100 * Math.cos(phi);
      ...
      helix.position.x = 3500 * Math.sin(phi);
      helix.position.y = - (i * 28) + 200;
      helix.position.z = 3500 * Math.cos(phi);
      ...
      grid.position.x = (( i % 5 ) * 900) - 800;
      grid.position.y = ( - ( Math.floor( i / 5 ) % 5 ) * 800 ) + 800;
      grid.position.z = (Math.floor( i / 25 )) * 2500 - 2000;
      d['grid'] = grid;
    }
