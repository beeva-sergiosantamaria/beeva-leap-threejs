...
angular.module('pruebaApp')
  ...
  .controller('RenderController', ['$scope',function($scope) {
    ...
    if (USE_VOICE && annyang) {
      ...
      var commands = {
        ...
        'categorías': function() {
          window.location = $("#enlace").children().attr('ng-href');
        },
        'pantalla completa': function() {
          toggleFullscreen();
        },
        'más datos': function() {
          addData(12);
    ...
    function addData(howMany){
      ...
      for ( var i = indexPointer; i < indexPointer + howMany; i += 1 ) {
        ...
        var test_data = stream_layers(3,CPS[i].categories.length,.1).map(function(data, i) {
          console.log(data);
          var valor;
          if(i===0) valor="ingresos";
          else if(i===1) valor="pagos";
          else if(i===2) valor="tarjetas usadas";
          return {
            key: valor,
            values: data
          };
        ...
        element.style.backgroundColor = 'rgba(187,187,189,'+( sumaIngresos * 0.0000001 ) + ')';

        var symbol = document.createElement('div');
        symbol.className = 'symbol';
        //symbol.textContent ='C.P.: '+CPS[i]._id;
        symbol.textContent = DISTRICTS[CPS[i]._id.toString()];
        ...
        var chart = nv.models.multiBarChart()
              ...
              .height(heigth)
              .stacked(true)
              .reduceXTicks(true) //si es falso, cada capa del eje x se renderiza.
              .rotateLabels(0)  //angulo de los valores de x.
              .showControls(true) //cambio entre grouped o stacked.
              .groupSpacing(0.1)  //distancia entre barras.
            ;
